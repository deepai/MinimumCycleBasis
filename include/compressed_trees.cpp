...
int compressed_trees::get_node_arrays(unsigned **csr_rows,unsigned **csr_cols,int **csr_edge_offset,int **csr_parent,
									  int **csr_distance,int node_index)
{
...
int compressed_trees::get_node_arrays_warp(unsigned **csr_rows,unsigned **csr_cols,int **csr_edge_offset,int **csr_parent,
...
void compressed_trees::print_tree()
{
	for(int i=0;i<chunk_size;i++)
	{
		...
		for(int j=0;j<original_nodes;j++)
		{
			...
			if(edge_offset != -1)
			{
				printf("%d - %d, %d\n",parent_graph->rows->at(edge_offset)+1,parent_graph->columns->at(edge_offset)+1,
